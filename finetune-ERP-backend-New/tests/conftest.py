...
def night_shift(db):
    from attendance.models import Shift
    return Shift.objects.create(name="Night", start_time=time(22, 0), end_time=time(6, 0), is_overnight=True)
...
def branch_head(db, store_s1):
    User = get_user_model()
    return User.objects.create_user(
        username="bh", password="pw", email="bh@example.com", role="branch_head", store=store_s1
    )
...
def advisor1(db, store_s1):
    ...
    User = get_user_model()
    return User.objects.create_user(
        username="adv1", password="pw", email="adv1@example.com", role="advisor", store=store_s1
    )
...
def advisor2(db, store_s2):
    User = get_user_model()
    return User.objects.create_user(
        username="adv2", password="pw", email="adv2@example.com", role="advisor", store=store_s2
    )
...
def fake_selfie():
    def _make(name="in.jpg"):
        # minimal JPEG bytes
        return SimpleUploadedFile(name, b"\xff\xd8\xff\xe0\x00\x00\xff\xd9", content_type="image/jpeg")
    return _make
