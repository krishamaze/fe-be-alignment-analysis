...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        group.add_argument('--dry-run', action='store_true', help='List issues without fixing (default)')
        group.add_argument('--apply', action='store_true', help='Apply fixes to simple mismatches')
    ...
    def handle(self, *args, **options):
        ...
        for store in Store.objects.select_related('branch_head').all():
            ...
            if bh:
                ...
                qs = CustomUser.objects.filter(store=store, role='branch_head', deleted=False, is_active=True)
                if qs.count() == 1:
                    mismatches += 1
                    user = qs.first()
                    if apply:
                        ...
                        self.stdout.write(f"Fix: set store {store.id} branch_head to user {user.id}")
                elif qs.count() > 1:
                    conflicts += 1
