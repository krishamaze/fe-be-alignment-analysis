...
| `PageWrapper`                                   | `src/components/layout`           | sets scroll mode (`reel` or `scroll`) and adds reel-mode bottom padding of `56px` plus safe-area inset                                                                                                                                                       |
...
Public routes wrap navigation inside a `ScrollModeProvider` with an outer `h-[100dvh] overflow-hidden` container and an inner `h-full relative flex flex-col`. `TopBar` scrolls away, `MainNav` is a `flex-shrink-0 h-14` element, content uses `flex-1 overflow-y-auto min-h-0`, and `BottomNav` anchors with `fixed bottom-0`. The provider tracks scroll direction and hides the nav after 100px of downward scrolling on mobile or 200px on desktop; upward scroll reveals it immediately on mobile or after 100px on desktop. Setting mode to `reel` keeps the nav visible and applies a fixed `56px` bottom padding plus safe-area inset.
