...
| ----------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `PublicLayout`                                  | `src/components/layout`           | wraps `TopBar`, `MainNav`, and `BottomNav` inside a `ScrollModeProvider`; outer `h-[100dvh] overflow-hidden` container with inner `relative flex flex-col` keeps navigation anchored |
...
Public routes wrap navigation inside a `ScrollModeProvider` with an outer `h-[100dvh] overflow-hidden` container and an inner `h-full relative flex flex-col`. `TopBar` scrolls away, `MainNav` is a `flex-shrink-0 h-14` element, content uses `flex-1 overflow-y-auto min-h-0`, and `BottomNav` anchors with `fixed bottom-0`.
