...
const StoreAssignModal = ({ isOpen, onClose, user }) => {
    ...
    const handleAssign = async () => {
        ...
        try {
            ...
            if (user.role === 'branch_head') {
                response = await dispatch(assignBranchHeadToStore({ storeId: Number(selectedStoreId), userId: user.id }));
            } else {
                response = await dispatch(assignStoreToUser({ userId: user.id, store: Number(selectedStoreId) }));
            }
    ...
    const handleUnassign = async () => {
        try {
            let response;
            if (user.role === 'branch_head') {
                response = await dispatch(unassignBranchHeadFromStore({ storeId: Number(selectedStoreId), userId: user.id }));
            } else {
