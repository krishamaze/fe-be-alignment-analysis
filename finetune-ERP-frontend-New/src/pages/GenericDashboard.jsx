...
function Section({ label, items, disabled = false }) {
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-body-sm font-medium text-primary/60 dark:text-surface/60">{label} ({items.length})</h2>
...
function GenericDashboard({ title, features }) {
  ...
  return (
    <div className="px-4 space-y-4">
      <h1 className="text-heading-md font-bold text-primary dark:text-surface">{title}</h1>
