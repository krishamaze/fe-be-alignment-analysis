...
This guide documents the UI component library, design system, and frontend development patterns for the Finetune ERP platform.

## Section Navigation Components

The following specialized navigation components enable section-based scrolling and slide interactions:
...
| ----------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `PublicLayout`                                  | `src/components/layout`           | wraps `TopBar`, `MainNav`, and mobile `BottomNav` inside a `ScrollModeProvider`; reveals a `Footer` on desktop after 85% scroll; uses a stable `h-[100dvh] overflow-hidden` container with a `main` region that adds `scrollPaddingTop` for the nav heights and conditional bottom padding for `BottomNav`. The `main` becomes scrollable and is registered as the scroll container only in scroll mode, and uses `overflow-hidden` in reel mode so `PageWrapper` provides its own container. |
...
| `Pagination` / `ResponsivePaginationHandler`    | `src/components`                  | pagination controls; mobile mode uses `IntersectionObserver` for infinite scroll                                                                                                                                                                                                                                                                                                                                                                             |
...
| `SectionSlider`                                 | `src/components/navigation`       | vertically stacked section slider with horizontal slide support, session-aware swipe hints, debounced slide detection, CSS scroll snapping, and ScrollModeContext integration                                                                                                                                                                                                                                                                                                                             |
...
- [Frontend Features](FRONTEND_FEATURES.md) – Booking and repair features
- [State Management](STATE_MANAGEMENT.md) – Redux state reference
