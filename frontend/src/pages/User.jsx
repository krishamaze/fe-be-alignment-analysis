...
const UserList = () => {
  ...
  const { totalPages, currentPage, totalElements, pageSize } = useAppSelector((state) => state?.user)
  ...
  const handleChange = (e) => {
    ...
    setUser(prev => {
      ...
      if (name === 'first_name' || name === 'last_name') {
        const firstNamePart = updatedUser.first_name.toLowerCase().slice(0, 3).padEnd(3, 'a');
        const lastNamePart = updatedUser.last_name.toLowerCase().slice(0, 1) || 'a';
        const randomPart = generateRandomString(2);
        updatedUser.username = `${firstNamePart}${lastNamePart}${randomPart}`.slice(0, 6);
      }
  ...
  const onEdit = (obj) => {
    const { id, first_name, last_name, username, email, password, phone, role, store } = obj;
    setUser({
      id: id,
              ...
              userData.map((user, index) => (
                ...
                >
                  <div className="mb-4">
                    ...
                    <p className="text-m text-gray-500 mt-2"><strong>Email:</strong> {user.email || '-'}</p>
                    <p className="text-m text-gray-500"><strong>Phone:</strong> {user.phone || '-'}</p>
                    <p className="text-m text-gray-500"><strong>Role:</strong> {user.role || '-'}</p>
                    <p className="text-m text-gray-500"><strong>Store:</strong> {user.store_name || <span className="text-m text-gray-400">unassigned</span>}</p>
                  </div>

                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                    <span onClick={() => handleToggleClick(user)} className="cursor-pointer">
                      {user.is_active ? (
                        <MdToggleOn className="text-green-500 hover:scale-110 transition-transform" size={26} />
                      ) : (
                        <MdToggleOff className="text-red-500 hover:scale-110 transition-transform" size={26} />
                      )}
                    </span>
                    <div className="flex gap-3 items-center">
                      ...
                      <button onClick={() => onEdit(user)} className="text-yellow-500 hover:text-yellow-600">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-end gap-4 pt-4">
                  ...
                  <button
                    ...
                    className={`w-full sm:w-auto px-4 py-2 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-keyline ${isSubmitting ? 'bg-gray-400' : 'bg-black hover:bg-gray-900'
                      }`}
                  >
